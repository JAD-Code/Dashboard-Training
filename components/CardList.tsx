...
const CardList = ({ title }: CardListProps) => {
  ...
  return (
    <div className="">
      <h1 className="text-lg font-medium mb-4 ms-2">{title}</h1>
      <div className="flex flex-col gap-2">
        {list.map((item) => (
          ...
          >
            <div className="w-12 h-12 rounded-lg relative overflow-hidden ms-2">
              <Image
                src={item.image}
                alt={item.title}
                fill
            ...
            <CardFooter className="p-0 me-2">{item.count / 1000}k</CardFooter>
          </Card>
        ))}
      </div>
    </div>
